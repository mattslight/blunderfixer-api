...
def analyse_pgn(
    request: AnalysePGNRequest,
    top_n: int = Query(3, ge=1, le=5),
    depth: int = Query(12, ge=1, le=40),
):
    try:
            ...
            for move in game.mainline_moves():
                ...
                try:
                    info_list = engine.analyse(
                        pre_move_board, chess.engine.Limit(depth=depth), multipv=top_n
                    )
                except Exception:
                    raise HTTPException(
                        status_code=500,
                        detail=f"Engine failed on move {move_number} (pre-analysis)",
                    )
                ...
                for info in info_list:
                    ...
                    top_moves.append(
                        TopMove(
                            san=pre_move_board.san(best_move), eval=round(eval_score, 2)
                        )
                    )
                ...
                # Eval AFTER move
                post_score = engine.analyse(board, chess.engine.Limit(depth=depth))[
                    "score"
                ].white()
